...
class Campuslife extends Model{
   ...
   public function uploadfiles(){
       ...
       if (empty($file)){
        ...
        $data = array();
            if ($_FILES["file"]["error"] > 0){
                ...
                $pic =  $this->upload();
                if($pic['info']== 1){
                    $url =request()->root(true).'/public'.'/uploads/'.'campuslife/'.$pic['savename'];
                    $imgpath='/public'.'/uploads/'.'campuslife/'.$pic['imgpath'];
    ...
    private  function upload(){
        $file = request()->file('file');
        $info = $file->move(ROOT_PATH . 'public' . DS . 'uploads'. DS . 'campuslife');
        $reubfo = array();
        if($info){
            $reubfo['info']= 1;
