...
class Campusvideo extends Model{
   ...
   public function uploadfiles(){
       ...
       if (empty($file)){
           return false;
            ...
            if ($_FILES["file"]["error"] > 0){
                ...
                $pic =  $this->upload();
                if($pic['info']== 1){
                    $url =request()->root(true).'/public'.'/uploads/'.'campusvideo/'.$pic['savename'];
                    $imgpath='/public'.'/uploads/'.'campusvideo/'.$pic['imgpath'];
    ...
    private  function upload(){
        $file = request()->file('file');
        $info = $file->move(ROOT_PATH . 'public' . DS . 'uploads'. DS . 'campusvideo');
        $reubfo = array();
        if($info){
            $reubfo['info']= 1;
