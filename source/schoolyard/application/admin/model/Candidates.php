...
class Candidates extends Model{
	public function Position(){
		return $this->belongsTo('positions','positionid','rid');
	}
   ...
   public function uploadfiles(){
       ...
       if (empty($file)){
        ...
        $data = array();
            if ($_FILES["file"]["error"] > 0){
                ...
                $pic =  $this->upload();
                if($pic['info']== 1){
                    $url =request()->root(true).'/public'.'/uploads/'.'candidates/'.$pic['savename'];
                    $filepath='/public'.'/uploads/'.'candidates/'.$pic['filepath'];
    ...
    private  function upload(){
        $file = request()->file('file');
        $info = $file->move(ROOT_PATH . 'public' . DS . 'uploads'. DS . 'candidates');
        $reubfo = array();
        if($info){
            $reubfo['info']= 1;
	...
	public function getPositions(){
		$sql="select * from zxcms_positions where status=0 and exists(select 1 from zxcms_recruitplan where status=0 and positionid=zxcms_positions.rid)";
		$data=Db::query($sql);
		return $data;
	}
}