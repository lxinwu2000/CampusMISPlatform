...
use think\Request;
use think\Db;
use app\admin\model\Campuslife;

class CampuslifeController extends CommonController{
    ...
    public function json(){
		...
		$pages=($page-1)*$limit;
		$data=Campuslife::where($where)->where('status',0)->limit($pages,$limit)->select();
		for($i=0;$i<count($data);$i++){
			if(empty($data[$i]['coverphoto'])){
				$data[$i]['coverphoto']='暂无';
			}else{
    ...
    public function edit(){
		...
		if(request()->isPost()){
			...
			$rid=input("get.rid");	
			$data=Campuslife::where("rid",$rid)->find();
