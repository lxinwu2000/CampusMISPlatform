...
layui.use('table', function(){
  ...
  table.render({    //表格渲染 
     elem: '#positionstable'
    ,url:Root+'admin/Studentrecruitmentbr/json'
    ,height: 'full-150'   
    ,cellMinWidth: 80 
    ...
    ,cols: [[
	  ...
	  ,{field:'cnname', width:150, title: '名称',align:'center'}
	  ,{field:'content', width:150, title: '内容详情',align:'center'}
      ,{field:'imgpath', width:150, title: '图片',align:'center'}
  ...
  table.on('tool(Idtableb)', function(obj){
	   ...
	   if(obj.event === 'del'){
	      layer.confirm('真的删除这条数据么', function(index){	    		    	
	    	 Ajaxalls(rid,null,2,'admin/Studentrecruitmentbr/delete');
//	    	 obj.del();
         ...
         ,getCheckid:function(){
        	 ...
        	 if(checkedid.length=='0'){
        		 ...
        		 layer.confirm('真的删除这些数据吗', function(index){       		    	
        		    	 $.ajax({
        		 			url : Root+"admin/Studentrecruitmentbr/delete",
        		 			type : "post",
