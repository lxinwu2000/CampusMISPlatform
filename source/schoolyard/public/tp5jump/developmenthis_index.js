...
layui.use('table', function(){
  ...
  table.render({    //表格渲染 
     elem: '#positionstable'
    ,url:Root+'admin/Developmenthis/json'
    ,height: 'full-150'   
    ,cellMinWidth: 80 
    ...
    ,cols: [[
      ...
      ,{field:'eventdate', width:150, title: '事件日期',align:'center'}
  ...
  table.on('tool(Idtableb)', function(obj){
	   ...
	   if(obj.event === 'del'){
	      layer.confirm('真的删除这条数据么', function(index){	    		    	
	    	 Ajaxalls(rid,null,2,'admin/Developmenthis/delete');
//	    	 obj.del();
         ...
         ,getCheckid:function(){
        	 ...
        	 if(checkedid.length=='0'){
        		 ...
        		 layer.confirm('真的删除这些数据吗', function(index){       		    	
        		    	 $.ajax({
        		 			url : Root+"admin/Developmenthis/delete",
        		 			type : "post",
